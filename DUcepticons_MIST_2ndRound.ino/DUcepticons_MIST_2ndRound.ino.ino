...
if(sonarFinishKey==0 && ringCompleteKey>1)
{
  ...
  //ring condition
  if(digitalReading[3]==1  && digitalReading [7]==1 && sonarFinishKey==0 )
  {
...
void caveCode()
{
  ...
  while(mappedValue==100)
  {
    ...
    if(difference>2 )
    {
      motor(caveSpeed+difference*100,caveSpeed);
      delay(100);
      Serial.print("Dane");      
    ...
    {
      motor(caveSpeed,caveSpeed-difference*100);
      delay(100);
      Serial.print("Bame");   
    }
    else 
    {
      motor(caveSpeed,caveSpeed);  //The minus because bot bends left
      delay(100);  
      Serial.print("Shoja");
    }
    sensorMapping();
